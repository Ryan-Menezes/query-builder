...
{
    public function makeSut(string $tableName): RightJoin
    {
        ...
        return new RightJoin($tableName, $sqlMock);
    }
    ...
    public function testShouldCreateARightJoinStatementCorrectly()
    {
        $sut = $this->makeSut('test');

        $this->assertEquals(
            'SELECT CONTACT(name, ?) FROM `any-table` RIGHT JOIN test',
            $sut->toSql(),
        );
    ...
    public function testShouldAcceptLogicalInstrutions()
    {
        ...
        $this->assertEquals(
            'SELECT CONTACT(name, ?) FROM `any-table` RIGHT JOIN test ON name = ?',
        ...
        $this->assertEquals(
            'SELECT CONTACT(name, ?) FROM `any-table` RIGHT JOIN test ON name = ? OR age = ?',

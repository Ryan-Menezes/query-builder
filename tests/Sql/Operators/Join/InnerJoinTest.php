...
{
    public function makeSut(string $tableName): InnerJoin
    {
        ...
        return new InnerJoin($tableName, $sqlMock);
    }
    ...
    public function testShouldCreateAnInnerJoinStatementCorrectly()
    {
    ...
    public function testShouldAcceptLogicalInstrutions()
    {
        ...
        $this->assertEquals(
            'SELECT CONTACT(name, ?) FROM `any-table` INNER JOIN test ON name = ? OR age = ?',
